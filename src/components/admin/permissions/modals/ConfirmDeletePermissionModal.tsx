...
export function ConfirmDeletePermissionModal({
    ...
    const handleConfirm = async () => {
        ...
        setLoading(true);
        try {
            const res = await PermmissionService.CallDeletePermission(permission.permissionId);
    ...
    return (
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent className="max-w-lg">
                <DialogHeader>
                    <DialogTitle className="text-xl font-semibold">Xác nhận xóa quyền hạn</DialogTitle>
                    <DialogDescription>
                        <span>
                            Bạn có chắc muốn xóa quyền hạn <b>{permission?.name}</b> không?
                            <br />
                            <span className="text-sm text-gray-600 mt-2 block">
                                Module: <b>{permission?.module}</b> | Method: <b>{permission?.method}</b>
                            </span>
