...
import { BadgePlus, ListOrdered, SquarePen, Trash2 } from "lucide-react";
...
import { DeleteForm } from "./part/deleteForm";
...
export const PartSection = ({ id }: { id: string }) => {
  ...
  const handeDeletePart = async () => {
    const res = await PartService.deletePart(selectedPart);
    if (res.EC === 0 && res.data) {
      toast.success(res.EM);
      handleFetchParts();
                ...
                <AccordionContent className="flex flex-col gap-4 text-balance p-4 min-h-[120px]">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <DeleteForm open={isDeleteOpen} onOpenChange={setIsDeletOpen} handleDeletePart={handeDeletePart} />
                    </div>
                  </div>
